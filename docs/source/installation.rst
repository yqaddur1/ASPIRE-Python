...
ASPIRE is based on Python 3.8, and comes with an ``environment.yml`` for reconstructing a working Conda environment to run the package.
...
ASPIRE should generally be compatible with newer version of Python, and newer dependent packages. We are currently testing 3.8, 3.9, 3.10, and 3.11 base Python as configured by ASPIRE, and with upgrading packages to the latest for each of those bases.
