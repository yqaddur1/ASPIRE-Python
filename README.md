...
ASPIRE is a pip-installable package that works on Linux/Mac/Windows, and requires Python 3.8. The simplest option is to use Anaconda 64-bit for your platform with a minimum of Python 3.8 and `pip`, and then use `pip` to install `aspire` in that environment.
...
If you prefer not to use Anaconda, or want to manage environments yourself, you should be able to use `pip` with Python >= 3.8.
